...
$sql = "SELECT c.id_categoria, c.nombre_categoria, sc.id_subcategoria, sc.nombre_subcategoria,ma.id_marca,ma.nombre_marca,fa.nombre_familia,mo.id_modelo,mo.nombre_modelo,mo.descripcion_modelo,mo.estado
FROM categoria c
...
ORDER BY  fa.nombre_familia, mo.nombre_modelo";
...
<body>
    ...
    <h2 class="text-center my-4">Lista de Modelos</h2>
    <div class="container">
                ...
                <tbody>
                    ...
                    while ($row_marca = $marca->fetch_assoc()) { ?>
                        <tr>
                            ...
                            <td><?= $row_marca['nombre_modelo']; ?></td>
                            <td class="text-start"><?= nl2br($row_marca['descripcion_modelo']); ?></td>
                            <td class="text-center">
                                <?php
                                // Verifica si el estado es activado para mostrar el enlace de editar
                                if ($row_marca['estado'] == 1) {
                                    echo "<a href='editar_modelo.php?id=" . $row_marca['id_modelo'] . "' class='btn btn-info mx-3'>Editar</a>";
                                }
    
                                if ($row_marca['estado'] == 1) {
                                    echo "<a href='../crud/estado_modelo.php?id=" . $row_marca['id_modelo'] . "&estado=0' class='btn btn-danger'>Desactivar</a>";
                                } else {
                                    echo "<a href='../crud/estado_modelo.php?id=" . $row_marca['id_modelo'] . "&estado=1' class='btn btn-success'>Activar</a>";
                                }
                                ?>
                            </td>
                        </tr>
